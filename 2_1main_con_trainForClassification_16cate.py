...
def parse_option():
    ...
    parser.add_argument('--data_folder', type=str, default='/home/tonglab/Datasets/imagenet16/train/', help='path to custom dataset')
    parser.add_argument('--val_data_folder', type=str, default='/home/tonglab/Datasets/imagenet16/val/', help='path to custom dataset')
    parser.add_argument('--size', type=int, default=224, help='parameter for RandomResizedCrop')

    # method
    parser.add_argument('--method', type=str, default='TempSupCon',
                        choices=['SupCon', 'SimCLR','TempSupCon'], help='choose method')
    ...
    opt.model_path = './TempSupCon/{}_models'.format(opt.dataset)
    opt.tb_path = './TempSupCon/{}_tensorboard'.format(opt.dataset)
    ...
    # opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_theLifeOfMammals30hz_threshold0.25'. \
    opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_theLifeOfMammals10hz_threshold0.5_length0.25'. \
        format(opt.method, opt.dataset, opt.model, opt.learning_rate,
               # opt.weight_decay, opt.batch_size, opt.temp, opt.trial)
               opt.weight_decay, 256, opt.temp, opt.trial)
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        mean = eval(opt.mean)
        std = eval(opt.std)
    elif opt.dataset == 'imageNet':#'imageNet':
        ...
        if (idx + 1) % opt.print_freq == 0:
            print('Train: [{0}][{1}/{2}]\t'
                  ...
                  'top5_accuracy {top5.val:.6f} ({top5.avg:.6f})'.format(
                   ...
                   data_time=data_time, loss=losses,loss_clf=losses_clf,
                   top1 = top1, top5 = top5))
            sys.stdout.flush()

    # return losses_ctr.avg,losses_clf.avg,losses.avg,top1.avg,top5.avg
    return losses_clf.avg,losses.avg,top1.avg,top5.avg
...
def main():
    ...
    load_path = os.path.join(opt.save_folder, 'ckpt_latest_trainForClassification_16cate.pth')
    tuneBefore = 1
    ...
    if os.path.isfile(load_path):
        ...
        if len(gpu_ids) <= 1: # 1. Multi-GPU to single-GPU or -CPU
            ...
            if tuneBefore == 0:
                new_state_dict['classifier.6.weight'] = new_state_dict['classifier.6.weight'][:16,:]
                new_state_dict['classifier.6.bias'] = new_state_dict['classifier.6.bias'][:16]
    ...
    if tuneBefore == 0:
        ...
        time2_val = time.time()
        stat_file = open(os.path.join(opt.save_folder, 'training_stats_trainForClassification_16cate.txt'), 'a+')
    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        loss_clf,loss,top1,top5 = train(train_loader, model, criterion_ctr,criterion_clf, optimizer, epoch, opt)
        time2 = time.time()
        time1_val = time.time()
        ...
        stat_str = '[Train] Epoch ({}) LR ({:.4f}) LossTotal ({:.4f}) LossClassification ({:.4f}) Top1Acc ({:.6f}) Top5Acc ({:.6f}) Time ({:.2f})'. \
            format(epoch, optimizer.param_groups[0]['lr'], loss, loss_clf, top1,top5, time2 - time1)
        stat_file.write(stat_str + '\n')
        stat_str = '[Validation] Epoch ({}) LR ({:.4f}) LossClassification ({:.4f}) Top1Acc ({:.6f}) Top5Acc ({:.6f}) Time ({:.2f})'. \
        ...
        if epoch % opt.save_freq == 0:
            save_file = os.path.join(
                opt.save_folder, 'ckpt_epoch_{epoch}_trainForClassification_16cate.pth'.format(epoch=epoch))
            save_model(model, optimizer, opt, epoch, save_file)

        save_file = os.path.join(
            opt.save_folder, 'ckpt_latest_trainForClassification_16cate.pth')
        save_model(model, optimizer, opt, epoch, save_file)
    # save the last model
    save_file = os.path.join(
        opt.save_folder, 'last_trainForClassification_16cate.pth')
