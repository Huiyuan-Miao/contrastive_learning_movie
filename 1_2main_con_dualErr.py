...
def main():
    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        stat_str = '[Train] Epoch ({}) LR ({:.4f}) LossTotal ({:.4f}) LossContrast ({:.4f}) LossClassificationd ({:.4f}) Time ({:.2f})'. \
            format(epoch, optimizer.param_groups[0]['lr'], loss, loss_ctr, loss_clf, time2 - time1)
        stat_file.write(stat_str + '\n')
        stat_str = '[Validation] Epoch ({}) LR ({:.4f}) LossClassificationd ({:.4f}) Top1Acc ({:.6f}) Top5Acc ({:.6f}) Time ({:.2f})'. \
