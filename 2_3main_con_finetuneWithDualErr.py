...
def parse_option():
    ...
    parser.add_argument('--learning_rate', type=float, default=0.001,
                        help='learning rate')
    parser.add_argument('--lr_decay_epochs', type=str, default='90,100',
    ...
    opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_blurTrained'.\
        format(opt.method, opt.dataset, opt.model, opt.learning_rate*10,
...
def train(train_loader, model, criterion_ctr,criterion_clf, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        loss = loss_ctr *0.9 +loss_clf*0.1
        # update metric
        losses.update(loss.item(), bsz)
        losses_ctr.update(loss_ctr.item(), bsz)
        losses_clf.update(loss_clf.item(), bsz*2)
...
def main():
    ...
    load_path = os.path.join(opt.save_folder, 'ckpt_latest_finetuneWithDualErr.pth')
    tuneBefore = 1
    ...
    if tuneBefore == 0:
        ...
        time2_val = time.time()
        stat_file = open(os.path.join(opt.save_folder, 'training_stats_finetuneWithDualErr.txt'), 'a+')
    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        if epoch % opt.save_freq == 0:
            save_file = os.path.join(
                opt.save_folder, 'ckpt_epoch_{epoch}_finetuneWithDualErr.pth'.format(epoch=epoch))
            save_model(model, optimizer, opt, epoch, save_file)

        save_file = os.path.join(
            opt.save_folder, 'ckpt_latest_finetuneWithDualErr.pth')
        save_model(model, optimizer, opt, epoch, save_file)
    # save the last model
    save_file = os.path.join(
        opt.save_folder, 'last_finetuneWithDualErr.pth')
