...
from utils_2 import ImageFolderWithPaths
from alexnet import *
from losses_temporal import SupConLoss
...
def parse_option():
    ...
    parser.add_argument('--data_folder', type=str, default='/media/tonglab/789aa685-f3c0-4b78-afb5-88d669eb7059/Miao/Data/theLifeOfMammals/subSample30hz_threshold0.5/', help='path to custom dataset')
    ...
    parser.add_argument('--timeLength', type=float, default='14',
                        help='samples within certain amount of time will be positive pairs, in unit of the id difference, 29 is 0.5s')
    ...
    opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_theLifeOfMammals30hz_threshold0.5_length{}'.\
        format(opt.method, opt.dataset, opt.model, opt.learning_rate,
               opt.weight_decay, opt.batch_size, opt.temp, opt.trial,(opt.timeLength+1)/60)
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        train_dataset = ImageFolderWithPaths(opt.data_folder,
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels, paths, indices) in enumerate(train_loader):
        data_time.update(time.time() - end)
        path_clean = torch.zeros((len(paths)))
        for i in range(len(paths)):
            path_clean[i] = cleanNum(paths[i])
        images = torch.cat([images[0], images[1]], dim=0)
        # images = torch.cat([images[0], add_blur_with(images[1])], dim=0)
        if torch.cuda.is_available():
            images = images.cuda(non_blocking=True)
            labels = labels.cuda(non_blocking=True)
            path_clean = path_clean.cuda(non_blocking=True)
        bsz = labels.shape[0]
        ...
        elif opt.method == 'TempSupCon':
            loss = criterion(features, labels = labels, time = path_clean, timeLength=opt.timeLength)
        else:
...
def cleanNum(a):
    return int(re.search(r'scene(.*?).png', a).group(1))

def add_gauss_noise(images):
