...
def parse_option():
    ...
    opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_theLifeOfMammals10hz'.\
...
def main():
    ...
    load_path = os.path.join(opt.save_folder, 'ckpt_latest_trainForClassification.pth')
    tuneBefore = 1
    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        if epoch % opt.save_freq == 0:
            save_file = os.path.join(
                opt.save_folder, 'ckpt_epoch_{epoch}_trainForClassification.pth'.format(epoch=epoch))
            save_model(model, optimizer, opt, epoch, save_file)

        save_file = os.path.join(
            opt.save_folder, 'ckpt_latest_trainForClassification.pth')
        save_model(model, optimizer, opt, epoch, save_file)
    # save the last model
    save_file = os.path.join(
        opt.save_folder, 'last_trainForClassification.pth')
